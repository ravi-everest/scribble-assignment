...
- **FR-010**: The guess history MUST record each guess's submitter name, the post-trim lowercased guess text, and whether the guess was correct or incorrect.
...
- **FR-013**: The system MUST NOT display any dedicated per-submission feedback message (e.g., "Correct!" or "Wrong!"); the correct/incorrect flag in the shared guess history is the sole result indicator.
- **FR-014**: All players (drawer and guessers) MUST see a shared scoreboard showing every player's current score, updated on each poll cycle.
...
- **Guess**: A single guess submission containing the guesser's name, the stored post-trim lowercased text, and a correct/incorrect flag.
...
- **SC-006**: All players see the same shared scoreboard (all players' scores) within two polling cycles of any score change.

## Clarifications

### Session 2026-06-19

- Q: Which version of guess text is stored and displayed in guess history — pre-trim or post-trim? → A: Post-trim (trimmed, lowercased text is stored and displayed).
- Q: Does the guesser receive immediate per-submission feedback (e.g., "Correct!" message) beyond seeing their entry in guess history? → A: No dedicated feedback message — result is conveyed solely via the correct/incorrect flag visible in the shared guess history.
- Q: Where are scores displayed — personal only, shared scoreboard, or not at all? → A: Shared scoreboard — all players' current scores are visible to everyone, updated each poll cycle.
